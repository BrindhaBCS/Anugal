...
class SAP_Tcode_Library:
    ...
    def max_scroll_down(self, table_id, number_of_visible_rows, max_rownum, max_scrolls=50):
        """
        Scrolls down a table in SAP until the last row is reached.

        :param table_id: The ID of the table to scroll down.
        :param number_of_visible_rows: The number of rows that should be visible at any given time.
        :param max_rownum: The maximum row number in the table.
        :param max_scrolls: The maximum number of scrolls to perform (default is 50).
        """
        # logger.info(f"Scrolling down table with ID '{table_id}'...")

        

        # Get the table object
        table = self.get_table_object(table_id)

        # Scroll down the table
        current_row = 1
        scrolls = 0
        while current_row <= max_rownum and scrolls < max_scrolls:
            # Scroll down the table
            table.scroll_down(number_of_visible_rows)

            # Get the current row number
            current_row = table.get_current_row_number()

            # Log the current row number
            logger.info(f"Current row number: {current_row}")

            # Increment the number of scrolls
            scrolls += 1

        # Log the result
        if current_row > max_rownum:
            logger.info(f"Reached the last row ({max_rownum}) in the table.")
        else:
            logger.info(f"Reached the maximum number of scrolls ({max_scrolls}).")
    ...
    def search_DoubleClick(self, user_area_id, search_text, max_scrolls=0):
        try:
            user_area = self.session.findById(user_area_id)
            scroll_count = 0
            found = False
    ...
    def find_table_rows(self, comp_id, search_comp): 
    ...
    def select_spam_based_on_text_2(self, control_id, search_texts):
        """
        Selects rows in a control based on the presence of specific texts in the specified columns.

        Args:
            control_id (str): The ID of the control to search in.
            search_texts (dict): A dictionary where keys are column names and values are lists of texts to search for.

        Returns:
            list: A list of row numbers where the search texts were found. If no matches are found, an empty list is returned.
        ...
        try:
            ...
            for row in range(row_count):
                ...
                # Check if any of the search_texts are found in the cell values
                for column, texts in search_texts.items():
                    cell_value = control.GetCellValue(row, column)
                    print(f"Cell value in column '{column}': {cell_value}")
                    
                    for text in texts:
                        if text in cell_value:
                            print(f"Text '{text}' found in row {row} and column '{column}'")
                            
                            # Select the row
                            control.SelectRow(row)  # Select the row
                            control.EnsureVisibleRow(row)  # Ensure the row is visible
                            control.SetFocus()  # Set focus to the control
                            
                            matching_rows.append(row)  # Add the row number to the list of matching rows
            
            if matching_rows:
                print(f"Matches found in rows: {matching_rows}")
    ...
    # def select_material_views(session, table_id, views, max_scrolls=50):
    #     table = session.findById(table_id)
    #     # user_area = session.findById(wnd[1]/usr/tblSAPLMGMMTC_VIEW/txtMSICHTAUSW-DYTXT)
    #     row_count = table.RowCount
    #     print(row_count)
    
    #     # Initialize a list to store found row values
    #     # found_rows = []
    
    #     # Iterate through the rows of the table
    #     for row in range(row_count):
    #         print(row)
    #         cell_id = f"wnd[1]/usr/tblSAPLMGMMTC_VIEW/txtMSICHTAUSW-DYTXT[0,{row}]"
    #         cell_value = session.findById(cell_id).text
    #         print(cell_value)
    #         if cell_value == views:
    #             print(row)
    ...
    def select_material_views(self, table_id, search_comp, visible_rows=17):
        """Process table rows and select rows based on search components."""
        rowcount = self.get_row_count(table_id)
        row_index = 0

        while row_index < rowcount:
            try:
                cell_id = f"{table_id}/txtMSICHTAUSW-DYTXT[0,{row_index}]"
                print(f"Trying to set focus on cell {cell_id}")
                
                # Try to set focus on the cell
                try:
                    self.session.findById(cell_id).setFocus()
                except:
                    pass  # Ignore if setting focus fails
                
                data = self.get_table_cell_text(table_id, row_index, 0)
                print(f"Row {row_index} Data: {data}")

                # Select row if data matches search components
                if data in search_comp:
                    self.select_table_row(table_id, row_index)

                row_index += 1

                # Page down if filled visible rows
                if row_index >= visible_rows:
                    row_index = 1  # Reset row index after page down
                    scroll_position = row_index * 16
                    self.scroll_table(table_id, scroll_position)
    ...
    #         return json_string  # Return the JSON data
